...
- 新增本地隔离的真实时钟性能基线、请求计数、有限运行观察、导入与图表交互验证脚本；不修改行情刷新、金融规则或动效策略。
- 开发验证：269 Node、build/typecheck、受影响lint、89浏览器检查记录及26性能比较门槛通过；独立GPT审计finding修复并VERIFY。发布阶段复用同一候选证据，不重复计算为新测试。
- 测量未证明帧率提速；主客户端块增加46字节。真机、真实后台和长时运行仍待补，完整2.7待办继续保留。[测量与限制](docs/PERFORMANCE_275.md)。
- 已发布Sites v26，源码`0a86366`，平台succeeded；`radar-v2.75`/`sites-v26`标记同一源码。[版本索引](docs/VERSIONS.md)记录发布与回退来源。
