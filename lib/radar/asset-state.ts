import { assetFor } from "../market";
import { CANDLE_LAG_ALLOWANCE, prepareMarketInput, rms, simpleReturns, type InputReason } from "./market-input";
...
type Dimension<T, M> = { label: string; message: string } & (
...
function unavailable<T, M>(reason: StateReason, message: string, metrics: M | null = null): Dimension<T, M> {
...
export function buildAssetState({ snapshot, symbol, now, enabled, online }: { snapshot: RadarSnapshot; symbol: string; now: number | undefined; enabled: boolean; online: boolean }): AssetState {
  ...
  const prepared = prepareMarketInput(snapshot, symbol, at);
  if (!prepared.ok) return fail(prepared.failure.reason, prepared.failure.message);
  const { bars, history, quote } = prepared.data;
  if (bars.some(point => point.time <= 0)) return fail("invalid_history", "历史时间无效");
  if (bars.some(point => point.time + history.intervalMs > at)) return fail("future_evidence", "K 线尚未结束，等待完整证据");
  const lastEnd = bars.at(-1)!.time + history.intervalMs;
  if (at - lastEnd > history.intervalMs + CANDLE_LAG_ALLOWANCE) return fail("stale_evidence", "最近完整 K 线已过期");
  ...
  let direction: Dimension<"upward" | "downward" | "no_direction", DirectionMetrics>;
  if (![net, netPercent, path, efficiency].every(Number.isFinite)) direction = unavailable("invalid_numeric", "方向指标无法形成有限数值");
  else {
    ...
    direction = { availability: "available", classification, label: { upward: "窗口偏上", downward: "窗口偏下", no_direction: "无明显单向结构" }[classification], reason: null,
      message: "20 个收盘间隔内的净变化与路径效率；描述已发生的方向结构，不代表趋势预测或震荡判断。",
      metrics: { netPercent, efficiency, path, minimumNetPercent: floor, minimumEfficiency: ASSET_STATE_RULES.efficiencyFloor } };
  }
  const returns = simpleReturns(window), current = returns.slice(-ASSET_STATE_RULES.currentReturns), baseline = returns.slice(0, ASSET_STATE_RULES.baselineReturns);
  const currentRmsPercent = rms(current), baselineRmsPercent = rms(baseline);
  let volatility: Dimension<"higher" | "lower" | "similar", VolatilityMetrics>;
  if (![...returns, currentRmsPercent, baselineRmsPercent].every(Number.isFinite)) volatility = unavailable("invalid_numeric", "RMS 指标无法形成有限数值");
  else {
    const metrics: VolatilityMetrics = { currentRmsPercent, baselineRmsPercent, ratio: null, minimumBaselinePercent: ASSET_STATE_RULES.rmsBaselineFloor, lowerRatio: ASSET_STATE_RULES.lowerRatio, higherRatio: ASSET_STATE_RULES.higherRatio, currentStartAt: window[ASSET_STATE_RULES.baselineReturns].time + history.intervalMs, baselineEndAt: window[ASSET_STATE_RULES.baselineReturns].time + history.intervalMs };
    if (baselineRmsPercent <= ASSET_STATE_RULES.rmsBaselineFloor) volatility = unavailable("baseline_too_small", "此前基线为零或过小，无法可靠比较倍数", metrics);
    else {
      const ratio = currentRmsPercent / baselineRmsPercent;
      if (!Number.isFinite(ratio)) volatility = unavailable("invalid_numeric", "RMS 比值无法形成有限数值");
      else {
        const classification = ratio >= ASSET_STATE_RULES.higherRatio ? "higher" : ratio <= ASSET_STATE_RULES.lowerRatio ? "lower" : "similar";
        volatility = { availability: "available", classification, label: { higher: "高于此前基线", lower: "低于此前基线", similar: "接近此前基线" }[classification], reason: null,
          message: "最近 4 个简单收益率的 RMS，与此前不重叠的 16 个比较；不是标准差或年化波动，较低不等于低风险。", metrics: { ...metrics, ratio } };
      }
    }
  }
