...
import { Activity, ArrowDownRight, ArrowUpRight, Bell, BellPlus, ChevronRight, Clock3, ExternalLink, Info, Loader2, Plus, Radar, RefreshCw, Star, Trash2, WifiOff, X, SlidersHorizontal, Cloud, Layers, ShieldCheck } from "lucide-react";
...
export default function MarketRadar(){
  ...
  useEffect(()=>{
    if(!hydrated)return;
    try{localStorage.setItem(STORAGE_KEY,JSON.stringify({watchlist,alerts,auto,backgroundTabs}));}
  ...
  useEffect(()=>{
    ...
    const historyTimer=visible?setInterval(()=>setHistoryRefresh(n=>n+1),60_000):undefined;
    return()=>{clearInterval(timer);clearInterval(historyTimer);};
  },[refresh,mayRun,visible,hydrated]);
  useEffect(()=>{
  ...
  const chartColor=chartPoints.length>1&&chartPoints[chartPoints.length-1].close<chartPoints[0].close?"#fb879a":"#adf28c";
  ...
  return <>
        ...
        <div className="left-column">
          <section className="panel chart-panel" id="price-chart" aria-label={`${selectedName}价格走势图`}>
            ...
            <Tabs value={range} onValueChange={value=>setRange(value as Range)}>
              <div className="period-tabs"><TabsList className="range-list" aria-label="走势时间范围">{PERIODS.map(p=><TabsTrigger key={p.value} value={p.value} className="range-trigger">{p.label}</TabsTrigger>)}</TabsList><span className="chart-legend"><span className="line-swatch" style={{background:chartColor}}/>价格走势</span></div>
              <TabsContent value={range}>
                {chartBusy?<div className="chart-empty"><Loader2 size={24} className="spin"/><span>正在获取真实行情…</span></div>:chartPoints.length<2?<div className="chart-empty"><Activity size={30}/><span>{chartProblem||"暂无足够走势数据"}</span><button className="small-link" onClick={refreshAll}>重新获取</button></div>:<div className="price-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartPoints} margin={{top:10,right:8,bottom:0,left:5}} accessibilityLayer><defs><linearGradient id="radar-price-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={chartColor} stopOpacity={.21}/><stop offset="96%" stopColor={chartColor} stopOpacity={0}/></linearGradient></defs><CartesianGrid vertical={false} stroke="#29313a" strokeDasharray="3 5"/><XAxis dataKey="time" tickFormatter={v=>new Intl.DateTimeFormat("zh-CN",range==="1d"?{hour:"2-digit",minute:"2-digit",hour12:false,timeZone:chartTimezone}:{month:"numeric",day:"numeric",timeZone:chartTimezone}).format(v)} minTickGap={60} tick={{fill:"#8593a3",fontSize:12}} axisLine={false} tickLine={false} dy={8}/><YAxis orientation="right" domain={["auto","auto"]} tickFormatter={v=>new Intl.NumberFormat("en-US",{maximumFractionDigits:Math.abs(v)<1?4:0}).format(v)} tick={{fill:"#8593a3",fontSize:12}} axisLine={false} tickLine={false} width={68} tickCount={4}/><Tooltip content={<ChartTip currency={quote?.currency??"USD"} timezone={chartTimezone}/>} cursor={{stroke:"#91ae73",strokeDasharray:"4 4"}}/><Area type="linear" dataKey="close" name="价格" stroke={chartColor} strokeWidth={2.3} fill="url(#radar-price-fill)" isAnimationActive={false} activeDot={{r:4,stroke:"#13171c",strokeWidth:2,fill:chartColor}}/></AreaChart></ResponsiveContainer></div>}
              </TabsContent>
            </Tabs>
    ...
    <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}><DialogContent className="modal-content settings-modal"><DialogHeader><span className="settings-emblem"><SlidersHorizontal size={23}/></span><DialogTitle>运行与通知</DialogTitle><DialogDescription>选择监控方式，清楚掌握雷达何时在运行。</DialogDescription></DialogHeader>
      ...
      <div className="cloud-status"><div><Cloud size={20}/><strong>云端持续监控</strong><span>尚未接入</span></div><p>关闭网页、电脑休眠或 iPhone 锁屏后的持续监控，需要连接定时运行的云服务与推送通道。当前开关不会开启云端任务。</p></div>
      <p className="settings-foot"><ShieldCheck size={14}/>设置保存在当前浏览器，可随时关闭。</p>
