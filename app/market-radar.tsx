...
export default function MarketRadar(){
  ...
  useEffect(()=>{
    ...
    const historyTimer=setInterval(()=>setHistoryRefresh(n=>n+1),60_000);
  ...
  return <>
    ...
    <Dialog open={infoOpen} onOpenChange={setInfoOpen}><DialogContent className="modal-content"><DialogHeader><DialogTitle>使用说明</DialogTitle><DialogDescription>了解行情时效、提醒范围和保存方式。</DialogDescription></DialogHeader><div className="info-list"><p><strong>行情：</strong>数据来自 Yahoo Finance，每 15 秒查询一次；免费接口可能延迟、限流或暂时不可用。价格旁显示行情源报价时间，取数失败会保留并标记上次价格。</p><p><strong>涨跌与走势：</strong>日涨跌相对行情源前收盘价，不是滚动 24 小时涨跌。股票显示常规交易时段；图表横轴使用交易所时区，其余时间使用设备本地时区。</p><p><strong>提醒：</strong>本页面处于前台并开启自动监控时，按有效报价判断。股票休市、取数失败或报价过期时不触发。免费行情延迟也会影响提醒时间。每条触发一次，重新开启开关可再用。</p><p><strong>保存：</strong>自选与提醒只保存在当前设备的当前浏览器，不会跨设备同步。系统通知需浏览器支持并授权，关闭网页后不会继续监控。</p><a href="https://help.yahoo.com/kb/SLN2310.html" target="_blank" rel="noreferrer">查看行情源交易所与延迟说明 <ExternalLink size={13} className="inline"/></a></div></DialogContent></Dialog>
  </>;
}
