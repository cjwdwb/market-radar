...
export default function MarketRadar(){
  ...
  const chartColor=chartPoints.length>1&&chartPoints[chartPoints.length-1].close<chartPoints[0].close?"#f6465d":"#00c087";
  ...
  return <>
    ...
    <main className="page">
      ...
      <footer className="footnote"><span><Activity size={13}/>{lastFetched?`最近检查 ${formatTime(lastFetched)} · ${loading?"更新中":active?"自动监控开启":"监控暂停"}`:"正在连接行情源"}<button className="small-link" style={{fontSize:12,marginLeft:6}} onClick={()=>setAuto(v=>!v)}>{auto?"暂停":"恢复"}</button></span><span>自选与提醒可同步云端 · 仅监控行情，不执行交易</span></footer>
    </main>
