...
export function CloudMonitor({watchlist,alerts}:{watchlist:string[];alerts:PriceAlert[]}) {
  ...
  const refresh=useCallback(async()=>{try{const res=await fetch('/api/monitor',{cache:'no-store',signal:AbortSignal.timeout(18000)});const data=await res.json();if(!res.ok)throw new Error(data.error);setState(data);setError("");}catch(e){setError(e instanceof Error?e.message:"云端连接失败");}},[]);
  ...
  async function save(enabled:boolean, sync:boolean){setBusy(true);try{const res=await fetch('/api/monitor',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled,watchlist:sync?watchlist:state?.watchlist??watchlist,alerts:sync?alerts:state?.alerts??alerts}),signal:AbortSignal.timeout(18000)});const data=await res.json();if(!res.ok)throw new Error(data.error);setState(data);setError("");}catch(e){setError(e instanceof Error?e.message:"同步失败");}finally{setBusy(false);}}
