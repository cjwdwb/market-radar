...
export const SignalCard = memo(function SignalCard({ event, onAsset, onWatch, onAlert, isWatched }: { event: RadarIntelligenceEvent; onAsset: (event: RadarIntelligenceEvent | string) => void; onWatch: (symbol: string) => void; onAlert: (symbol: string) => void; isWatched: boolean }) {
  ...
  return <article className={`radar-signal signal-${event.status} ${event.kind !== "signal" ? "signal-cluster" : ""}`} aria-label={`${displaySymbol(event.symbol)} ${event.title}`}>
    ...
    <details className="signal-context"><summary>Why triggered · 依据与来源</summary>
      <div className="signal-explanation"><p>{primary.evidence?.reason ?? primary.description}</p>
        ...
        <div className="signal-actions"><button className="btn btn-primary" onClick={() => onAsset(event)}>查看图表</button><button className="btn" onClick={() => onWatch(event.symbol)}>{isWatched ? "移出自选" : "加入自选"}</button><button className="btn" onClick={() => onAlert(event.symbol)}>设置提醒</button></div>
...
export function RadarFeed({ intelligence, coverage, watchlist, scanning, loading, online, onAsset, onWatch, onAlert, onAdd, onRemove }: Props) {
