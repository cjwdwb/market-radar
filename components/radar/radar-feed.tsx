...
const preciseNumber = new Intl.NumberFormat("zh-CN", { maximumSignificantDigits: 10 });
...
function evidenceValue(item: RadarEvidenceItem) { return typeof item.value === "number" ? `${(item.unit ? number : preciseNumber).format(item.value)}${item.unit ?? ""}` : item.value; }
...
export function RadarFeed({ intelligence, coverage, watchlist, scanning, loading, online, onAsset, onWatch, onAlert, onAdd, onRemove, assetContext, onClearContext, priceAlertCounts }: Props) {
  ...
  const filtered = intelligence.events.filter(event => (showHistory || event.status === "active") && (assetContext ? event.symbol === assetContext.symbol && (event.status !== "active" || assetContext.activeEvents.includes(event)) : filter === "all" || filter === "watchlist" && watchlist.includes(event.symbol) || filter === "priority" && event.severity !== "medium" || filter === event.market));
