...
export function CandleChart({ points, currency, timezone }: { points: Point[]; currency: string; timezone: string }) {
  ...
  const candles = points.filter(p => p.open != null && p.high != null && p.low != null).slice(-96);
  if (!candles.length) return <div className="chart-empty">暂无有效 K 线数据</div>;
  const width = 900, left = 12, right = 94, plot = width - left - right;
  ...
  return <div className="candlestick-panel">
    ...
    <svg viewBox="0 0 900 354" role="img" aria-label="15 分钟 K 线与成交量，左右方向键逐根查看" tabIndex={0} style={{ width: "100%", minHeight: 240 }}
      ...
      {candles.map((p, i) => { const color = p.close >= p.open! ? "#adf28c" : "#fb879a", xx = x(i), w = Math.max(1, step * .65), volume = (p.volume ?? 0) / volMax * 58; return <g key={p.time} opacity={p.confirmed === false ? .65 : 1}><line x1={xx} x2={xx} y1={y(p.high!)} y2={y(p.low!)} stroke={color}/><rect x={xx - w / 2} y={Math.min(y(p.open!), y(p.close))} width={w} height={Math.max(1, Math.abs(y(p.open!) - y(p.close)))} fill={color}/><rect x={xx - w / 2} y={322 - volume} width={w} height={Math.max(.5, volume)} fill={color} opacity={.7}/></g>; })}
