...
export function CandleChart({ points, currency, timezone }: { points: Point[]; currency: string; timezone: string }) {
  ...
  return <div className="candlestick-panel" ref={container}>
    ...
    <svg viewBox={`0 0 ${width} 354`} role="img" aria-label="15 分钟 K 线与成交量，左右方向键逐根查看" tabIndex={0} style={{ width: "100%", display: "block", touchAction: "pan-y" }}
      ...
      {candles.map((p, i) => { const color = p.close >= p.open! ? "#e8e8e8" : "#969696", xx = x(i), w = Math.max(1, step * .65), volume = (p.volume ?? 0) / volMax * 58; return <g key={p.time} opacity={p.confirmed === false ? .72 : 1}><line x1={xx} x2={xx} y1={y(p.high!)} y2={y(p.low!)} stroke={color}/><rect x={xx - w / 2} y={Math.min(y(p.open!), y(p.close))} width={w} height={Math.max(1, Math.abs(y(p.open!) - y(p.close)))} fill={p.close >= p.open! ? color : "#0d0d0d"} stroke={color} strokeWidth={1}/>{volumeVisible && <rect data-volume="true" x={xx - w / 2} y={322 - volume} width={w} height={Math.max(.5, volume)} fill={color} opacity={.45}/>}</g>; })}
      {averages && <g fill="none" strokeWidth="1.3"><path data-ma="7" d={line(7)} stroke="#c9c9c9"/><path data-ma="25" d={line(25)} stroke="#787878" strokeDasharray="4 4"/></g>}
      <line x1={left} x2={width - right} y1={y(last.close)} y2={y(last.close)} stroke={last.close >= last.open! ? "#e8e8e8" : "#969696"} strokeDasharray="4 4" opacity=".6"/>
      <rect x={width - right + 3} y={y(last.close) - 10} width={right - 5} height={20} rx={2} fill={last.close >= last.open! ? "#ededed" : "#b5b5b5"}/><text x={width - right + 8} y={y(last.close) + 4} fill="#111" fontSize="11">{price(last.close, currency, false)}</text>
      <line x1={x(index)} x2={x(index)} y1={10} y2={323} stroke="#888888" strokeDasharray="3 3"/>
      {[0, Math.floor(candles.length / 2), candles.length - 1].filter((_, i) => width >= 600 || i !== 1).map(i => <text key={i} x={x(i)} y={346} textAnchor={i === 0 ? "start" : i === candles.length - 1 ? "end" : "middle"} fill="#858585" fontSize="11">{time(candles[i].time)}</text>)}
    </svg>
