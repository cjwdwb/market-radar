...
export const CandleChart=memo(function CandleChart({ points, currency, timezone }: { points: Point[]; currency: string; timezone: string }) {
  ...
  const geometry=useMemo(()=>buildGeometry(candles,sums,width,count,offset,offset,averages,volumeVisible,timeFormatter),[candles,sums,width,count,offset,averages,volumeVisible,timeFormatter]);
...
function buildGeometry(candles:Point[],sums:number[],width:number,count:number,start:number,offset:number,averages:boolean,volumeVisible:boolean,timeFormatter:Intl.DateTimeFormat){
