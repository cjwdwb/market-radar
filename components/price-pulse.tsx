...
import { isMotionReduced, MOTION_CHANGE_EVENT } from "@/lib/motion-preference";
...
export const PricePulse = memo(function PricePulse({value, text, identity}:{value:number|undefined;text:string;identity:string}) {
  ...
  useEffect(() => {
    ...
    const stop = () => { if (isMotionReduced() || document.hidden) animation.cancel(); };
    window.addEventListener(MOTION_CHANGE_EVENT,stop);
    document.addEventListener("visibilitychange",stop);
    return () => { animation.cancel(); window.removeEventListener(MOTION_CHANGE_EVENT,stop); document.removeEventListener("visibilitychange",stop); };
  }, [value, identity]);
