...
2026-09-22最新用户决定：“之前的未认证的不用管了，全面推进”。旧QA-A保留未完成的历史事实，本轮不再安排或作为优化Gate；不把它改为PASS、不绕过门禁。当前本地精修见tasks/archive/MR-STATE25-REFINE.md。后续生产操作仍需要相应明确授权。

## 后续补证（MR-MARKET-STATE-25 Planner，2026-09-22）

QA-B原NOT RUN更新为**PASS（用户真机反馈）**：用户确认在iPhone16 Pro/iOS26.2、正式2.45实测原第5步，明确答复“已实测，四项都正常”（资产正确、手动周期保留、返回可用、限制清楚）。无代理真机操作或截图证据，不扩展为2.5新UI通过。QA-A仍**BLOCKED**：再次打开独立Edge到门禁后，用户说明当前无法使用Edge；停止该尝试，未取得授权工作台，不绕过访问码。整体仍证据不足。安排与Builder Gate见 `tasks/archive/MR-MARKET-STATE-25.md`；以下为先前验收完成时的历史记录，原五步反馈继续保留。
